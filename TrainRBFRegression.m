...
%
%  Where z_i is an observed pixel value at location p_i=[x_i; y_i]
%
%
%  Each RBF has a center c_k=[x_k; y_k] and width 'sig'
%
%
...
function [w]=TrainRBFRegression(z,P,C,sig)
  for i = 1:size(C,2)
    B(:,i) = rbf2d(P, C(:,i), sig)'; % eval set of points at different basis funcs
  end;
  B = [B, ones(size(B,1),1)]; % add bias term to B
  w = B\z; % find the weights
return;
